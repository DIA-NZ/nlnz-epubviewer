...
/**
 * EpubViewerManagerServlet handles all http requests for epub files
 * 
 * Input parameters:
...
public class EpubViewerManagerServlet extends BaseAbstractServlet {

	private static final long serialVersionUID = 1031295532865380959L;
	private static Logger logger = Logger.getLogger(EpubViewerManagerServlet.class);

	public EpubViewerManagerServlet() {
		super();
	}
	...
	protected void processRequest(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (req != null) {
			...
			String rosettaSession = req.getParameter(ROSETTA_SESSION_REQUEST_PARAMETER);

			logger.info("Processing request for FILEPID:[" + filePid + "], ROSETTA SESSION:[" + rosettaSession + "]");

			if (filePid != null && rosettaSession != null) {

				DeliveryAccessWS deliveryWS = null;
				String rosettaFilePath = null;

				try {
					// Initialize the Web Service call
					deliveryWS = new DeliveryAccessWS_Service(new URL("http://" + ndhaDeliveryServer + DELIVERY_WSDL_URL),
							new QName("http://dps.exlibris.com/", "DeliveryAccessWS")).getDeliveryAccessWSPort();
					logger.debug("processRequest() - Delivery WS initialized");

					if (deliveryWS != null) {
						// Retrieve the epub file path
						rosettaFilePath = deliveryWS.getFilePathByDvs(rosettaSession, filePid);
						...
						if (req.getParameter(DOWNLOADFILE_REQUEST_PARAMETER) != null &&
							...
							logger.info("Processing Download file request for FILEPID:[" + filePid + "], ROSETTA SESSION:[" + rosettaSession + "]");
							
							getFileFromRosetta(res, rosettaFilePath, filePid);
							...
							if (epubFileName != null && clientIP.isEmpty() == false) {
								...
								req.setAttribute("rosettaSession", rosettaSession);
						...
						logger.error("Delivery web service initialization failed. Null object returned.");
						
						errorMessage = "Delivery web service call returned null for FilePid:[" + filePid + "].";
						reportError(req, res, errorMessage);
					}
	 ...
	 * @param filePid
	 */
	private void getFileFromRosetta(HttpServletResponse res, String epubFilePath, String filePid) {
		...
		try {
			...
			if (epubFile != null && epubFile.exists()) {
				...
				logger.info("getFileFromRosetta() - Requested filename: " + epubFilename);
		        ...
		        logger.info("getFileFromRosetta() - MIME Type set to: " + mimetype);
		        
				bis = new BufferedInputStream(new FileInputStream(epubFile));
				sos = res.getOutputStream();
				byte[] buffer = new byte[1024 * 8];
				int readSize = -1;
				logger.info("getFileFromRosetta() - buffering the requested file to servlet output stream...start");
				while ((readSize = bis.read(buffer)) != -1) {
					sos.write(buffer, 0, readSize);
				}
				logger.info("getFileFromRosetta() - buffering the requested file to servlet output stream...end");
				if (sos != null) {
					sos.close();
				}
				if (bis != null) {
